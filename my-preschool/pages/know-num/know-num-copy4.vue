<template>
  <view class="container">
    ...
    <!-- 游戏难度选择 -->
    <picker mode="selector" :range="gameLevels" range-key="title" @change="onLevelChange" v-if="currentGameType.id === 'a1'">
    ...
    uni.getSystemInfo({
      success: (res) => {
        ...
        if (currentGameType.value.id === 'a1') {
          numSum = parseInt(currentLevel.value.numSum)
        } else {
          // 其他游戏类型根据可用项目计算
          const maxPairs = currentGameType.value.id === 'a2' 
            ? changeColorList.value.length 
            : changeAnimalList.value.length * (currentGameType.value.id === 'a4' ? 1 : 1)
          
          // 计算最大可能的项目数，不超过屏幕能容纳的数量
          const maxScreenItems = Math.floor((screenInfo.value.safeArea.right - screenInfo.value.safeArea.left - 40) / 50) * 
                                Math.floor((screenInfo.value.safeArea.bottom - screenInfo.value.safeArea.top - 200) / 50)
          
          numSum = Math.min(maxPairs * 2, maxScreenItems)
          // 确保项目数是偶数
          numSum = numSum % 2 === 0 ? numSum : numSum - 1
        }
...
const generateBoard = () => {
  ...
  switch (currentGameType.value.id) {
    case 'a1': // 数字消消乐
      const numSum = Math.min(parseInt(currentLevel.value.numSum), colNum.value * rowNum.value)
    ...
    case 'a2': // 颜色消消乐
      const colorCount = Math.min(changeColorList.value.length, Math.floor(colNum.value * rowNum.value / 2))
    ...
    case 'a3': // 动物消消乐
      const animalCount = Math.min(changeAnimalList.value.length, Math.floor(colNum.value * rowNum.value / 2))
    ...
    case 'a4': // 动物和影子消消乐
      const shadowAnimalCount = Math.min(changeAnimalList.value.length, Math.floor(colNum.value * rowNum.value / 2))
      const selectedShadowAnimals = [...changeAnimalList.value]
        .sort(() => Math.random() - 0.5)
        .slice(0, shadowAnimalCount)
      
      selectedShadowAnimals.forEach(animal => {
  ...
  // 填充剩余位置（如果有）
  while (pairs.length < colNum.value * rowNum.value) {
    pairs.push(null)
  }
  
  // 随机打乱
  pairs = pairs.filter(item => item !== null).sort(() => Math.random() - 0.5)
